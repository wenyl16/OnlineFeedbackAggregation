...
def solve_decomposition_strictly_step2(p_target_total, p_base, ess_df, pv_df, ev_list, tcl_list,
    ...
    if (results.solver.status == SolverStatus.ok) and (
        ...
        abs_error = np.sum(np.abs(delta_p0 - delta_p_star))

        return abs_error, delta_p_star, p_actual_total, ess_powers, pv_powers, ev_powers, tcl_powers
...
def rolling_power_decomposition():
    ...
    alpha = 1.0  # 功率边界缩放系数
    beta = 1.0  # 能量边界缩放系数
    learning_rate1 = 5e-10
    learning_rate2 = 1e-12
    ...
    while current_time + T <= total_simulation_periods:
        ...
        # 3. 求解分解问题
        (abs_error, delta_p_star, p_actual_agg,
        ...
        if current_loss > 1e-4:
            ...
            print(
                f"  [反馈] Loss={current_loss:.4f}, Grad_A={grad_alpha:.4f}, Grad_B={grad_beta:.4f} -> New Alpha={alpha:.3f}, Beta={beta:.3f}")
