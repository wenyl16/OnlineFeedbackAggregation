...
def update_parameters_step5_strict(alpha, beta, grad_alpha, grad_beta, learning_rate1,learning_rate2):
    ...
    """
    max_step_alpha = 0.005
    max_step_beta = 0.005
    step_alpha = learning_rate1 * grad_alpha
    step_beta = learning_rate2 * grad_beta
    step_alpha = np.clip(step_alpha, -max_step_alpha, max_step_alpha)
    step_beta = np.clip(step_beta, -max_step_beta, max_step_beta)
    # 核心更新逻辑
...
def rolling_power_decomposition():
    ...
    while current_time + T <= total_simulation_periods:
        ...
        # 3. 求解分解问题
        (sqrd_error, delta_p_star, p_actual_agg,
        ...
        if sqrd_error > 1e-4:
            ...
            print(
                f"  [反馈] Loss={sqrd_error:.4f}, Grad_A={grad_alpha:.4f}, Grad_B={grad_beta:.4f} -> New Alpha={alpha:.3f}, Beta={beta:.3f}")
...
if __name__ == '__main__':
    ...
    if histories is not None and len(histories["ess_power"]) > 0:
        ...
        # 保存绘图数据为 .mat 文件
        # =========================================================
        scipy.io.savemat('plot_data_zero_grad.mat', {
